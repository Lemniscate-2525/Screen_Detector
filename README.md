...
4. I initially tried a *first principles approach* (FFT-based detection, glare, local texture uniformity, straight-edge detection) and tested individually, in pairs, and combined via stacking-style logistic regression. After two rounds of debugging (removing manual clipping thresholds, fixing feature scaling), this approach plateaued around 60-65% cross-validated accuracy even with different feature combinations or classifier choices(logistic regression, RandomForest, and SVM all converged to a similar accuracy value).
...
1. More data; 100 images is very small for a held-out evaluation that needs to generalize across hardware I haven't seen, more images across more phone models and screen types would def inc accuracy and probably get a better generalization.

2. I'd also validate whether a smaller backbone (MobileNetV3-Small) maintains accuracy while cutting latency, since 215ms is acceptable but not "instant" for users on a phone.
